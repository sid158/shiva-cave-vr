import"./modulepreload-polyfill-B5Qt9EMX.js";import{aF as I,ae as z,an as j,M as D,C as re,Y as k,E as H,a as M,ak as O,W as Y,G as N,ah as ie,V as R,aC as se,S as le,aD as ce,aE as ue,X as de,aG as ve}from"./three-B0MzMegT.js";function J(n){let e=n>>>0;return()=>(e=e*1664525+1013904223>>>0,e/4294967296)}function me(n){const e=new I(500,24,16),t=new z({side:j,transparent:!0,depthWrite:!1,uniforms:{uOpacity:{value:0}},vertexShader:`
      varying vec3 vDir;
      void main() {
...
`;function he(n,{count:e=5200}={}){const t=J(20260805),a=new Float32Array(e*3),r=new Float32Array(e),m=new Float32Array(e),u=new Float32Array(e),d=new Float32Array(e*3),p=new re;for(let c=0;c<e;c++){let g=t()*2-1;const y=t()*Math.PI*2;t()<.42&&(g*=.3);const x=Math.sqrt(Math.max(0,1-g*g)),E=90+t()*320;a[c*3]=Math.cos(y)*x*E,a[c*3+1]=g*E,a[c*3+2]=Math.sin(y)*x*E;const F=Math.pow(t(),2.4);m[c]=.85+F*4.2,r[c]=k.clamp(.86-F*.9+(t()-.5)*.22,0,.9),u[c]=t()*100;const P=t();P>.86?p.setHSL(.07+t()*.04,.55,.72):P>.66?p.setHSL(.12+t()*.03,.22,.85):p.setHSL(.58+t()*.06,.28+t()*.3,.88),d[c*3]=p.r,d[c*3+1]=p.g,d[c*3+2]=p.b}const v=new H;v.setAttribute("position",new M(a,3)),v.setAttribute("aDelay",new M(r,1)),v.setAttribute("aSize",new M(m,1)),v.setAttribute("aSeed",new M(u,1)),v.setAttribute("aTint",new M(d,3));const o={uEmerge:{value:0},uTime:{value:0},uPixel:{value:1}},l=new z({vertexShader:fe,fragmentShader:pe,uniforms:o,transparent:!0,depthWrite:!1,depthTest:!1,blending:O}),f=new Y(v,l);f.frustumCulled=!1,f.renderOrder=5;const w=new N;return w.add(f),n.add(w),{group:w,uniforms:o,set emerge(c){o.uEmerge.value=c},get emerge(){return o.uEmerge.value},update(c,g){o.uTime.value=c,o.uPixel.value=g,w.rotation.y=c*.0042,w.rotation.x=Math.sin(c*.017)*.014}}}const ge=`
  attribute float aSeed;
  attribute float aSize;
  uniform float uTime;
  uniform float uPixel;
...
`;function ye(n,{count:e=260,radius:t=3}={}){const a=J(77003),r=new Float32Array(e*3),m=new Float32Array(e),u=new Float32Array(e);for(let l=0;l<e;l++)r[l*3]=(a()-.5)*t*2.2,r[l*3+1]=.25+a()*2.3,r[l*3+2]=(a()-.5)*t*2.2,m[l]=a(),u[l]=.5+a()*1.4;const d=new H;d.setAttribute("position",new M(r,3)),d.setAttribute("aSeed",new M(m,1)),d.setAttribute("aSize",new M(u,1));const p={uTime:{value:0},uPixel:{value:1},uFade:{value:0}},v=new z({vertexShader:ge,fragmentShader:we,uniforms:p,transparent:!0,depthWrite:!1,blending:O}),o=new Y(d,v);return o.frustumCulled=!1,o.renderOrder=3,n.add(o),{points:o,set fade(l){p.uFade.value=l},get fade(){return p.uFade.value},update(l,f){p.uTime.value=l,p.uPixel.value=f}}}const xe=`
  precision highp float;
...
`;function Me(n){const e={uTime:{value:0},uLevel:{value:0},uFade:{value:0}},t=new z({vertexShader:Se,fragmentShader:Te,uniforms:e,transparent:!0,depthWrite:!1,depthTest:!1,blending:O}),a=new D(new ie(.44,.44),t);a.frustumCulled=!1,a.renderOrder=30,n.add(a);let r=null,m=null,u=0;const d=new R(0,1.35,-1.7),p=new R,v=new R(0,1.35,-1.7);return a.position.copy(v),{mesh:a,uniforms:e,attachAnalyser(o,l){r=o.createAnalyser(),r.fftSize=512,r.smoothingTimeConstant=.55,m=new Uint8Array(r.frequencyBinCount),l.connect(r)},set fade(o){e.uFade.value=o},get fade(){return e.uFade.value},update(o,l,f){e.uTime.value=o,f.getWorldDirection(p),f.getWorldPosition(d),d.addScaledVector(p,1.7);const w=1-Math.exp(-l*5.5);v.lerp(d,w),a.position.set(v.x,v.y+Math.sin(o*.9)*.008,v.z);let c=0;if(r){r.getByteFrequencyData(m);const y=Math.max(8,m.length*.45|0);let A=0;for(let x=2;x<y;x++)A+=m[x];c=Math.min(1,A/(y-2)/132)}const g=c>u?1-Math.exp(-l*22):1-Math.exp(-l*3.2);u+=(c-u)*g,e.uLevel.value=u}}}const b={motesIn:[34,64],motesFull:[64,78],dim:[78,94],stars:[101,152],nebula:[116,176],drone:[76,96]},Ae="assets/audio/journey_01.mp3",_=4,Ee=[83.5,93],W=(n,e,t)=>n+(e-n)*t,S=(n,[e,t])=>k.clamp((n-e)/(t-e),0,1),T=n=>n*n*(3-2*n);function C(n,e){const t=document.getElementById("status");t&&(t.textContent=n);const a=document.getElementById("substatus");a&&e!==void 0&&(a.textContent=e)}function Fe(n,e){const t=n.createGain();t.gain.value=0,t.connect(e);const a=n.createBiquadFilter();a.type="lowpass",a.frequency.value=220,a.Q.value=.6,a.connect(t);for(const[r,m]of[[38,.55],[57,.28],[76.4,.18]]){const u=n.createOscillator();u.type="sine",u.frequency.value=r;const d=n.createGain();d.gain.value=m,u.connect(d).connect(a),u.start()}return{set level(r){t.gain.setTargetAtTime(r*.09,n.currentTime,.25)}}}async function Pe(){const n=document.getElementById("view"),e=new se({canvas:n,antialias:!0,alpha:!0,powerPreference:"high-performance"});e.setPixelRatio(Math.min(2,window.devicePixelRatio)),e.setSize(window.innerWidth,window.innerHeight),e.setClearAlpha(0),e.xr.enabled=!0,e.xr.setReferenceSpaceType("local-floor"),e.outputColorSpace=le,e.toneMapping=ce,e.toneMappingExposure=1.15;const t=new ue;t.background=null;const a=new de(70,window.innerWidth/window.innerHeight,.05,900);a.position.set(0,1.35,0);const r=new N;r.add(a),t.add(r);const m=me(t),u=he(t),d=be(u.group),p=ye(t),v=Me(t);window.addEventListener("resize",()=>{a.aspect=window.innerWidth/window.innerHeight,a.updateProjectionMatrix(),e.setSize(window.innerWidth,window.innerHeight)});const o=new Audio;o.src=Ae,o.preload="auto",o.crossOrigin="anonymous";let l=!1;o.addEventListener("canplaythrough",()=>{l=!0},{once:!0}),o.addEventListener("error",()=>{console.warn("[journey] voiceover failed to load"),l=!0},{once:!0});let f=null,w=null;function c(){if(!f)try{f=new(window.AudioContext||window.webkitAudioContext);const s=f.createGain();s.gain.value=1,s.connect(f.destination);const h=f.createMediaElementSource(o),i=f.createGain();i.gain.value=1,h.connect(i).connect(s),v.attachAnalyser(f,i),w=Fe(f,s)}catch(s){console.warn("[journey] web audio unavailable, voice only",s)}}let g=!1,y=0,A=84.4;o.addEventListener("loadedmetadata",()=>{isFinite(o.duration)&&(A=o.duration)});let x=null;function E(){return x!==null?x:g?!o.ended&&o.currentTime>.01?(y=performance.now()/1e3-o.currentTime,o.currentTime):(o.ended||o.currentTime>=A-.05,performance.now()/1e3-y):-_}let F="flat";async function P(){if(!navigator.xr)return null;try{if(await navigator.xr.isSessionSupported("immersive-ar"))return"immersive-ar"}catch{}try{if(await navigator.xr.isSessionSupported("immersive-vr"))return"immersive-vr"}catch{}return null}async function Q(){c(),f&&f.state==="suspended"&&await f.resume();const s=await P();if(s){const h={requiredFeatures:["local-floor"],optionalFeatures:["hand-tracking"]};try{const i=await navigator.xr.requestSession(s,h);await e.xr.setSession(i),F=s,i.addEventListener("end",()=>{F="flat"})}catch(i){console.warn("[journey] could not start",s,i)}}document.body.classList.add("started"),g=!0,y=performance.now()/1e3+_,setTimeout(()=>{o.play().catch(h=>console.warn("[journey] autoplay blocked",h))},_*1e3)}const B=new R;let G=-_;e.setAnimationLoop((s,h)=>{const i=E(),X=k.clamp(i-G,.001,.1);G=i;const Z=e.getContext(),q=Math.max(.5,Z.drawingBufferHeight/900),K=W(0,.55,T(S(i,b.motesIn)))+W(0,.45,T(S(i,b.motesFull)));p.fade=K*(1-T(S(i,[b.dim[0],b.dim[0]+9]))),p.update(i,q);const $=S(i,b.dim);m.opacity=Math.pow($,1.6),u.emerge=S(i,b.stars),u.update(i,q),d.emerge=T(S(i,b.nebula)),d.update(i);const V=T(S(i,[b.dim[0]+8,b.dim[1]+8]));B.set(Math.sin(i*.031)*.55,Math.sin(i*.019+1.7)*.42,Math.cos(i*.024)*.55).multiplyScalar(V),r.position.copy(B),r.rotation.z=Math.sin(i*.013)*.03*V;const ee=e.xr.isPresenting?e.xr.getCamera():a,te=T(S(i,[-_+.8,-.2])),ae=1-T(S(i,Ee));if(v.fade=te*ae,v.update(i,X,ee),w){const ne=T(S(i,b.drone)),oe=1-.45*T(S(i,[98,126]));w.level=ne*oe}e.render(t,a)});const U=await P();U==="immersive-ar"?C("Put your headset on and begin.","You will start in your own room."):U==="immersive-vr"?C("Passthrough unavailable — begin anyway.","Your room will be represented rather than seen."):C("Preview in browser","Open this page in the Quest browser for the full experience.");const L=document.getElementById("begin");L.disabled=!1,L.addEventListener("click",async()=>{L.disabled=!0,l||(C("Loading the voice…"),await new Promise(s=>{if(l)return s();const h=setInterval(()=>{l&&(clearInterval(h),s())},100);setTimeout(()=>{clearInterval(h),s()},6e3)})),await Q()}),window.JOURNEY={THREE:ve,scene:t,camera:a,renderer:e,stars:u,nebula:d,voidShell:m,motes:p,narrator:v,T:b,seek:s=>{x=s},resume:()=>{x=null},look:(s,h=0)=>{a.rotation.set(h,s,0,"YXZ")},moveTo:(s,h,i)=>{r.position.set(0,0,0),a.position.set(s,h,i)},dryStart:()=>{g=!0,y=performance.now()/1e3,document.body.classList.add("started")},fakeLevel:s=>{v.uniforms.uLevel.value=s},state:()=>({t:E(),xrMode:F,voidOpacity:m.opacity,starEmerge:u.emerge,nebulaEmerge:d.emerge,orbFade:v.fade})},window.__JOURNEY_READY=!0}Pe().catch(n=>{console.error(n),window.__JOURNEY_ERROR=String(n&&n.stack||n),C("Something went wrong.",String(n))});
