import{G as pt,M as gt}from"./meshopt_decoder.module-D7UQsJ41.js";import{$ as je,g as D,o as pe,M as K,C as wt,p as ke,a3 as Ue,a as $,A as X,a4 as Ee,G as Ge,n as we,P as Ce,V as Q,N as Me,c as Be,D as ct,a5 as xt,a6 as yt,B as bt,b as Ne,I as tt,f as at,Q as St,a7 as Tt,Y as Ft,S as _t,Z as At,_ as Mt,x as kt,a0 as Ut}from"./three-nyODTqkC.js";function ut(l){let t=l>>>0;return()=>(t=t*1664525+1013904223>>>0,t/4294967296)}function Et(l){const t=new je(500,24,16),a=new D({side:pe,transparent:!0,depthWrite:!1,uniforms:{uOpacity:{value:0}},vertexShader:`
      varying vec3 vDir;
      void main() {
    ...
    `}),i=new K(t,a);return i.frustumCulled=!1,i.renderOrder=-10,l.add(i),{mesh:i,set opacity(c){a.uniforms.uOpacity.value=c},get opacity(){return a.uniforms.uOpacity.value}}}const Ct=`
...
`;function ot(l,{radius:t=430,scale:a=2.1,gain:i=1,order:c=4}={}){const x=new je(t,48,32),u={uTime:{value:0},uEmerge:{value:0},uPalette:{value:0},uFull:{value:0},uRadial:{value:0},uScale:{value:a},uGain:{value:i},uHell:{value:0}},g=new D({side:pe,transparent:!0,depthWrite:!1,depthTest:!0,blending:X,uniforms:u,vertexShader:`
      varying vec3 vDir;
      void main() {
        vDir = position;
        gl_Position = projectionMatrix * modelViewMatrix * vec4(position, 1.0);
      }
    `,fragmentShader:Pt}),s=new K(x,g);return s.frustumCulled=!1,s.renderOrder=c,l.add(s),{mesh:s,set emerge(v){u.uEmerge.value=v},get emerge(){return u.uEmerge.value},set palette(v){u.uPalette.value=v},set full(v){u.uFull.value=v},set radial(v){u.uRadial.value=v},set hell(v){u.uHell.value=v},update(v){u.uTime.value=v}}}const Gt=`
  precision highp float;
...
`;function It(l){const t=new we(150,150,240,64,1,!0),a={uTime:{value:0},uEmerge:{value:0}},i=new D({side:pe,transparent:!0,depthWrite:!1,depthTest:!1,blending:X,uniforms:a,vertexShader:`
      varying vec2 vUv;
      varying float vY;
      void main() {
        vUv = uv;
    ...
    `,fragmentShader:Gt}),c=new K(t,i);return c.frustumCulled=!1,c.renderOrder=5,l.add(c),{mesh:c,set emerge(x){a.uEmerge.value=x},update(x){a.uTime.value=x,c.rotation.y=x*.006}}}const qt=`
  attribute float aSeed;
  attribute float aSize;
  uniform float uTime;
  uniform float uPixel;
...
`;function Qt(l){const t=Bt(90210),a=850,i=new Float32Array(a),c=new Float32Array(a*3),x=new Float32Array(a),u=new Float32Array(a),g=new Q(0,.05,-1).normalize(),s=new Q;for(let y=0;y<a;y++){i[y]=t();do s.set(t()*2-1,t()*2-1,t()*2-1);while(s.lengthSq()>1||s.lengthSq()<.05);s.normalize(),t()<.25&&s.lerp(g,.6).normalize(),c[y*3]=s.x,c[y*3+1]=s.y,c[y*3+2]=s.z,x[y]=2.2+Math.pow(t(),.7)*5.8,u[y]=1.8+t()*3.4}const v=new Ue;v.setAttribute("position",new $(new Float32Array(a*3),3)),v.setAttribute("aSeed",new $(i,1)),v.setAttribute("aDir",new $(c,3)),v.setAttribute("aRad",new $(x,1)),v.setAttribute("aSize",new $(u,1));const b={uTime:{value:0},uIn:{value:0},uField:{value:0},uPixel:{value:1},uNose:{value:new Q(0,1.3,-.3)}},S=new Ee(v,new D({vertexShader:Wt,fragmentShader:jt,uniforms:b,transparent:!0,depthWrite:!1,depthTest:!1,blending:X}));S.frustumCulled=!1,S.renderOrder=24,l.add(S);const k=700,_=new Float32Array(k),m=new Float32Array(k*3);for(let y=0;y<k;y++){_[y]=t();do s.set(t()*2-1,t()*2-1,t()*2-1);while(s.lengthSq()>1||s.lengthSq()<.05);s.normalize(),m[y*3]=s.x,m[y*3+1]=s.y,m[y*3+2]=s.z}const T=new Ue;T.setAttribute("position",new $(new Float32Array(k*3),3)),T.setAttribute("aSeed",new $(_,1)),T.setAttribute("aDir",new $(m,3));const O={uTime:{value:0},uOut:{value:0},uPixel:{value:1},uNose:{value:new Q(0,1.3,-.3)},uFwd:{value:new Q(0,0,-1)}},n=new Ee(T,new D({vertexShader:Ht,fragmentShader:Yt,uniforms:O,transparent:!0,depthWrite:!1,depthTest:!1,blending:X}));n.frustumCulled=!1,n.renderOrder=25,l.add(n);const C={uTime:{value:0},uCore:{value:0},uIn:{value:0}},P=new K(new Ce(16,16),new D({vertexShader:rt,fragmentShader:$t,uniforms:C,transparent:!0,depthWrite:!1,depthTest:!1,blending:X}));P.position.set(0,2.5,-14),P.frustumCulled=!1,P.renderOrder=6,l.add(P);const z={uTime:{value:0},uCore:{value:0}},V=new K(new Ce(5,90),new D({vertexShader:rt,fragmentShader:`
        precision highp float;
        uniform float uTime;
        uniform float uCore;
        varying vec2 vUv;
      ...
      `,uniforms:z,transparent:!0,depthWrite:!1,depthTest:!1,blending:X}));V.position.set(0,2.5,-14.2),V.frustumCulled=!1,V.renderOrder=5,l.add(V);const N=new Q,ee=new Q;let H=0,J=0;return{get breathing(){return Math.max(H,J)},set core(y){C.uCore.value=y,z.uCore.value=y},set field(y){b.uField.value=y},gateIn:0,gateOut:0,update(y,o,w,p){const G=1-Math.exp(-o*(this.gateIn>H?3.2:1.8));H+=(this.gateIn-H)*G;const f=1-Math.exp(-o*(this.gateOut>J?9:4));J+=(this.gateOut-J)*f,w.getWorldPosition(N),w.getWorldDirection(ee),N.addScaledVector(ee,.12),N.y-=.05,b.uTime.value=y,b.uPixel.value=p,b.uIn.value=H,b.uNose.value.copy(N),O.uTime.value=y,O.uPixel.value=p,O.uOut.value=J,O.uNose.value.copy(N),O.uFwd.value.copy(ee),C.uTime.value=y,C.uIn.value=H,z.uTime.value=y}}}const Kt=`
  varying vec2 vUv;
  void main() {
    vUv = uv * 2.0 - 1.0;
    gl_Position = projectionMatrix * modelViewMatrix * vec4(position, 1.0);
...
`;function he(l,t,a,i,{blending:c=X,order:x=8}={}){const u=new D({vertexShader:Kt,fragmentShader:a,uniforms:i,transparent:!0,depthWrite:!1,depthTest:!1,blending:c,side:ct}),g=new K(new Ce(l,t),u);return g.frustumCulled=!1,g.renderOrder=x,g}function na(l){const t=new Ge;l.add(t);const a={uTime:{value:0},uForm:{value:0},uOpen:{value:0},uSuck:{value:0},uFlare:{value:0}},i=Jt,c=Zt.replace("__COMMON__",i).replace("__NOISE__",it),x=Xt.replace("__COMMON__",i).replace("__NOISE__",it),u=260,g=220,s=-190,v=55,b=he(u,g,c,a,{blending:Me,order:7});b.position.set(0,v,s),t.add(b);const S=he(u,g,x,a,{order:8});S.position.set(0,v,s),t.add(S);const k=new Q(0,v+.38*(g/2),s),_=he(34,34,ea,{uTime:a.uTime,uForm:{value:0}},{order:8});_.position.set(95,130,-180),t.add(_);const m=he(70,70,nt,{uTime:a.uTime,uForm:{value:0},uSpin:{value:.01}},{order:7});m.position.set(-150,45,-175),t.add(m);const T=he(44,44,nt,{uTime:a.uTime,uForm:{value:0},uSpin:{value:-.014}},{order:7});T.position.set(150,20,-165),t.add(T);const O={uForm:{value:0}},n=new K(new we(90,90,34,96,1,!0),new D({vertexShader:`
        varying vec2 vUv;
        void main() {
          vUv = uv;
          gl_Position = projectionMatrix * modelViewMatrix * vec4(position, 1.0);
        }
      `,fragmentShader:aa,uniforms:O,transparent:!0,depthWrite:!1,depthTest:!1,blending:Me,side:pe}));n.position.y=-14,n.renderOrder=9,n.frustumCulled=!1,t.add(n);const C={uTime:a.uTime,uForm:{value:0}},P=new K(new we(110,110,60,96,1,!0),new D({vertexShader:`
        varying vec2 vUv;
        void main() {
          vUv = uv;
          gl_Position = projectionMatrix * modelViewMatrix * vec4(position, 1.0);
        }
      `,fragmentShader:oa,uniforms:C,transparent:!0,depthWrite:!1,depthTest:!1,blending:X,side:pe}));P.position.y=-12,P.renderOrder=6,P.frustumCulled=!1,t.add(P);const z=420,V=new Float32Array(z*3),N=new Float32Array(z);for(let r=0;r<z;r++){const Y=Math.random()*Math.PI*2,te=40+Math.random()*55;V[r*3]=Math.cos(Y)*te,V[r*3+1]=-20+Math.random()*30,V[r*3+2]=Math.sin(Y)*te,N[r]=Math.random()}const ee=new Ue;ee.setAttribute("position",new $(V,3)),ee.setAttribute("aSeed",new $(N,1));const H={uTime:a.uTime,uForm:{value:0},uPixel:{value:1}},J=new Ee(ee,new D({vertexShader:`
      attribute float aSeed;
    ...
    `,uniforms:H,transparent:!0,depthWrite:!1,depthTest:!1,blending:X}));J.frustumCulled=!1,J.renderOrder=9,t.add(J);const y={uTime:{value:0},uShow:{value:0},uTidal:{value:0},uDir:{value:new Be(1,-.1)},uSpin:{value:.05}},o={uTime:{value:0},uShow:{value:0},uTidal:{value:0},uDir:{value:new Be(-1,.12)},uSpin:{value:-.065}},w=he(120,120,lt,y,{order:22}),p=he(95,95,lt,o,{order:22});w.position.set(-58,18,-135),p.position.set(58,30,-140),l.add(w),l.add(p),w.visible=p.visible=!1;const G={uTime:{value:0},uRing:{value:0}},f=he(420,420,ta,G,{order:23});f.position.set(0,24,-138),l.add(f),f.visible=!1;const U={uTime:{value:0},uGrow:{value:0}},E=he(7,7,ia,U,{order:26});E.position.set(0,1.5,-9),l.add(E);const I={uTime:{value:0},uRush:{value:0}},A=new K(new we(3.2,3.2,130,40,1,!0),new D({vertexShader:`
        varying vec2 vUv;
        void main() {
          vUv = uv;
          gl_Position = projectionMatrix * modelViewMatrix * vec4(position, 1.0);
        }
      `,fragmentShader:ra,uniforms:I,transparent:!0,depthWrite:!1,depthTest:!1,blending:X,side:pe}));A.rotation.x=Math.PI/2,A.position.set(0,1.5,-20),A.renderOrder=27,A.frustumCulled=!1,l.add(A);const B={uBlack:{value:0},uWhite:{value:0}},R=new K(new je(.6,16,12),new D({vertexShader:`
      ...
      `,uniforms:B,transparent:!0,depthWrite:!1,depthTest:!1,blending:Me,side:pe}));return R.renderOrder=50,R.frustumCulled=!1,l.add(R),t.visible=!1,{group:t,eyeWorld:k,set collide(r){const Y=r>.001&&r<1;w.visible=p.visible=Y,y.uShow.value=o.uShow.value=Math.min(1,r*4),y.uTidal.value=o.uTidal.value=Math.pow(r,1.6);const te=58*(1-Math.pow(r,1.7)*.86);w.position.x=-te,p.position.x=te,p.position.y=30-Math.pow(r,1.7)*10},set flash(r){B.uWhite.value=r},set ring(r){G.uRing.value=r,f.visible=r>.002&&r<.999},set flare(r){a.uFlare.value=r},set fireSurge(r){C.uForm.value=Math.min(1.6,C.uForm.value*(1+r))},set portal(r){U.uGrow.value=r,E.visible=r>.002},set rush(r){I.uRush.value=r,A.visible=r>.002},set form(r){a.uForm.value=r,t.visible=r>.001||a.uSuck.value>.001,_.material.uniforms.uForm.value=Math.min(1,r*2.2),m.material.uniforms.uForm.value=Math.min(1,Math.max(0,r*1.8-.15)),T.material.uniforms.uForm.value=Math.min(1,Math.max(0,r*1.8-.25)),O.uForm.value=Math.min(1,r*2.6),C.uForm.value=Math.min(1,Math.max(0,r*2.2-.2)),H.uForm.value=Math.min(1,Math.max(0,r*2-.3))},set open(r){a.uOpen.value=r},set suck(r){a.uSuck.value=r;const Y=1+r*r*7;t.scale.setScalar(Y),t.position.copy(k).multiplyScalar(1-Y)},set black(r){B.uBlack.value=r},update(r,Y,te){a.uTime.value=r,U.uTime.value=r,I.uTime.value=r,y.uTime.value=r,o.uTime.value=r,G.uTime.value=r,H.uPixel.value=Y,R.visible!==B.uBlack.value>.003&&(R.visible=B.uBlack.value>.003),te.getWorldPosition(R.position),A.position.x=R.position.x,A.position.y=R.position.y}}}const We=`
  float hash(vec2 p) { return fract(sin(dot(p, vec2(127.1, 311.7))) * 43758.5453); }
  float noise(vec2 p) {
    vec2 i = floor(p), f = fract(p);
    f = f * f * (3.0 - 2.0 * f);
...
`;function Fe(l,t,a,i,{blending:c=X,order:x=8,vert:u=Pe,side:g=ct}={}){const s=new D({vertexShader:u,fragmentShader:a.replace("__NOISE__",We),uniforms:i,transparent:!0,depthWrite:!1,blending:c,side:g}),v=new K(new Ce(l,t),s);return v.frustumCulled=!1,v.renderOrder=x,v}function pa(l){const t=new Ge;t.visible=!1,l.add(t);const a=[],i=[],c=new xt(3151370,1.6);t.add(c);const x=[];for(let o=0;o<4;o++){const w=new yt(16734744,20,26,1.8);t.add(w),x.push(w)}const u={uTime:{value:0},uFire:{value:1}};a.push(u),i.push(u);const g=new K(new bt(3.6,.35,170),new D({vertexShader:Pe,fragmentShader:ca.replace("__NOISE__",We),uniforms:u}));g.position.set(0,-.18,-70),t.add(g);const s=new we(.055,.075,.95,7),v=new Ne({color:1511436,roughness:.9}),b=new tt(s,v,68),S=new at;let k=0;for(let o=4;o>=-148;o-=4.5)for(const w of[-1.9,1.9]){if(k>=68)break;S.makeTranslation(w,.45,o),b.setMatrixAt(k++,S)}b.instanceMatrix.needsUpdate=!0,t.add(b);for(const[o,w,p,G]of[[210,90,.75,3],[330,150,.9,2]]){const f={uForm:{value:1},uTall:{value:p}},U=new K(new we(o,o,w,96,1,!0),new D({vertexShader:Pe,fragmentShader:ua,uniforms:f,transparent:!0,depthWrite:!1,side:pe}));U.position.y=w*.32,U.renderOrder=G,U.frustumCulled=!1,t.add(U)}const _={uTime:{value:0},uForm:{value:1}};a.push(_),i.push(_);const m=new K(new we(360,360,200,96,1,!0),new D({vertexShader:Pe,fragmentShader:fa.replace("__NOISE__",We),uniforms:_,transparent:!0,depthWrite:!1,depthTest:!1,blending:X,side:pe}));m.position.y=30,m.renderOrder=1,m.frustumCulled=!1,t.add(m);const T=[];for(let o=0;o<8;o++){const w={uTime:{value:0},uForm:{value:1},uSeed:{value:o/8}};a.push(w),i.push(w);const p=Fe(16,190,va,w,{order:4}),G=o/8*Math.PI*2+.4,f=130+o%3*55;p.position.set(Math.cos(G)*f,60,Math.sin(G)*f-40),p.rotation.y=-G+Math.PI/2,t.add(p),T.push(p)}const O={uTime:{value:0},uWatch:{value:0}},n={uTime:{value:0},uWatch:{value:0},uLook:{value:new Be(0,0)}};a.push(O,n);const C=Fe(150,110,da,O,{blending:Me,order:5});C.position.set(0,78,-235),t.add(C);const P=Fe(150,110,ma,n,{order:6});P.position.set(0,78,-234),t.add(P);const z={uTime:{value:0},uForm:{value:1},uRelease:{value:0}};a.push(z);const V=Fe(60,44,ha,z,{blending:Me,order:7});V.position.set(0,15,-165),t.add(V);const N=[];for(let o=0;o<14;o++){const w=o%2===0?-1:1,p=-6-o*10.5-o%3*1.5;N.push(new Q(w*(5.3+o%3*.9),0,p))}const ee=[],H=[],J=new pt().setMeshoptDecoder(gt);J.load("assets/models/cauldron.glb",o=>{let w=null;if(o.scene.traverse(R=>{R.isMesh&&!w&&(w=R)}),!w)return;const p=w.geometry;p.computeBoundingBox();const G=p.boundingBox,f=new Q;G.getSize(f);const U=2.4/Math.max(f.x,f.z),E=new Ne({color:2299922,roughness:.93,metalness:.25}),I=new tt(p,E,N.length),A=new at;N.forEach((R,r)=>{A.compose(new Q(R.x,-G.min.y*U-.15,R.z),new St().setFromEuler(new Tt(0,r*1.7%6.28,0)),new Q(U,U,U)),I.setMatrixAt(r,A)}),I.instanceMatrix.needsUpdate=!0,I.frustumCulled=!1,t.add(I);const B=f.y*U;N.forEach((R,r)=>{const Y={uTime:{value:0},uFire:{value:1},uSeed:{value:r*.37%1}};a.push(Y),i.push(Y);const te=Fe(2.4,3,sa,Y,{order:9,vert:la});te.position.set(R.x,B+.9,R.z),t.add(te)}),x.forEach((R,r)=>{const Y=N[r*3]||N[0];R.position.set(Y.x,B+1,Y.z)})}),J.load("assets/models/figure.glb",o=>{let w=null;if(o.scene.traverse(E=>{E.isMesh&&!w&&(w=E)}),!w)return;const p=w.geometry;p.computeBoundingBox();const G=new Q;p.boundingBox.getSize(G);const f=1.75/G.y,U=new Ne({color:1182472,roughness:1});for(let E=0;E<N.length;E++){const I=1+E%2;for(let A=0;A<I;A++){const B=new K(p,U),R=N[E];B.scale.setScalar(f*(.9+A*.13)),B.position.set(R.x+(A?.45:-.3),.55,R.z+(A?-.2:.25)),B.rotation.y=(E*2.1+A*2.8)%6.28,B.frustumCulled=!1,t.add(B),ee.push({mesh:B,seed:E*1.3+A*7.7,baseY:B.position.y})}}for(let E=0;E<4;E++){const I=new K(p,U);I.scale.setScalar(f*2.2),I.frustumCulled=!1,t.add(I),H.push({mesh:I,col:T[E*2],seed:E*3.3})}});const y=new Q;return{group:t,set reveal(o){t.visible=o>.002},set watch(o){O.uWatch.value=o,n.uWatch.value=o},set release(o){for(const p of i)p.uFire&&(p.uFire.value=1-o);for(const p of i)p.uForm&&(p.uForm.value=1-o*.85);z.uRelease.value=o;const w=1-o*.9;c.intensity=1.6*w+o*2.2,c.color.setRGB(.19+o*.7,.09+o*.75,.04+o*.8);for(const p of x)p.intensity=20*w},update(o,w){for(const f of a)f.uTime&&(f.uTime.value=o);w.getWorldPosition(y);const p=ke.clamp((y.x-0)/60,-1,1),G=ke.clamp((y.y-40)/90,-1,.4);n.uLook.value.set(p,G);for(const f of ee){const U=Math.sin(o*.9+f.seed);f.mesh.rotation.z=U*.14,f.mesh.rotation.x=Math.sin(o*.6+f.seed*1.7)*.1,f.mesh.position.y=f.baseY+Math.sin(o*.5+f.seed)*.1}for(const f of H){const U=(o*.14+f.seed)%1;f.mesh.position.copy(f.col.position),f.mesh.position.y=140-U*170,f.mesh.rotation.set(o*.8+f.seed,o*.5,o*.7+f.seed)}for(let f=0;f<x.length;f++){const U=x[f];U.intensity>.2&&(U.intensity=U.intensity*.92+(16+Math.sin(o*7+f*2.3)*3+Math.sin(o*13.7+f)*2)*.08)}}}}const ie={motesIn:[34,64],motesFull:[64,78],dim:[70,84.5],stars:[85.5,104],nebula:[90,112],drone:[68,88]},j=116,q={start:j,url:"assets/audio/journey_02.mp3",starsOut:[j+5,j+17],palette:[j+6,j+21],full:[j+6,j+23],curtains:[j+10,j+26],core:[j+18.5,j+24.5],field:[j+20,j+28],breathIn:[j+28.9,j+33.3],breathOut:[j+33.6,j+38.5],loopFrom:j+66,loopPeriod:11.5,loopAmp:.62},W=j+64,F={voiceUrl:"assets/audio/journey_03.mp3",voiceAt:W,collide:[W+.3,W+6.8],blast:W+6.85,throw:[W+7,W+11],world:[W+9.5,W+16],drumFrom:W+23.5,form:[W+32.2,W+47.5],flare:[W+48.4,W+51],open:[W+60.8,W+65.5],suck:[W+64.5,W+70.5],black:[W+69.5,W+71.5]},ae=253.5,oe={voiceUrl:"assets/audio/journey_04.mp3",voiceAt:ae+2,reveal:[ae,ae+4.5],walk:[ae+3,ae+62],watch:[ae+24.4,ae+30.5],mantra:ae+57.2,release:[ae+57.2,ae+61.5],white:[ae+59.5,ae+64.5],end:ae+68},ga="assets/audio/journey_01.mp3",_e=4,wa=[82.5,88.5],st=(l,t,a)=>l+(t-l)*a,d=(l,[t,a])=>ke.clamp((l-t)/(a-t),0,1),h=l=>l*l*(3-2*l);function Ae(l,t){const a=document.getElementById("status");a&&(a.textContent=l);const i=document.getElementById("substatus");i&&t!==void 0&&(i.textContent=t)}function xa(l,t){const a=l.createGain();a.gain.value=0,a.connect(t);const i=l.createBiquadFilter();i.type="lowpass",i.frequency.value=220,i.Q.value=.6,i.connect(a);for(const[c,x]of[[38,.55],[57,.28],[76.4,.18]]){const u=l.createOscillator();u.type="sine",u.frequency.value=c;const g=l.createGain();g.gain.value=x,u.connect(g).connect(i),u.start()}return{set level(c){a.gain.setTargetAtTime(c*.09,l.currentTime,.25)}}}async function ya(){const l=document.getElementById("view"),t=new Ft({canvas:l,antialias:!0,alpha:!0,powerPreference:"high-performance"});t.setPixelRatio(Math.min(2,window.devicePixelRatio)),t.setSize(window.innerWidth,window.innerHeight),t.setClearAlpha(0),t.xr.enabled=!0,t.xr.setReferenceSpaceType("local-floor"),t.outputColorSpace=_t,t.toneMapping=At,t.toneMappingExposure=1.15;const a=new Mt;a.background=null;const i=new kt(70,window.innerWidth/window.innerHeight,.05,900);i.position.set(0,1.35,0);const c=new Ge;c.add(i),a.add(c);const x=Et(a),u=Ot(a),g=ot(u.group,{radius:460,scale:.85,gain:.7,order:-2}),s=ot(u.group,{radius:300,scale:1.9,gain:1,order:-1}),v=It(u.group),b=zt(a),S=Nt(a),k=Qt(a),_=na(a),m=pa(a);window.addEventListener("resize",()=>{i.aspect=window.innerWidth/window.innerHeight,i.updateProjectionMatrix(),t.setSize(window.innerWidth,window.innerHeight)});const T=new Audio;T.src=ga,T.preload="auto",T.crossOrigin="anonymous";let O=!1;T.addEventListener("canplaythrough",()=>{O=!0},{once:!0}),T.addEventListener("error",()=>{console.warn("[journey] segment-1 voice failed to load"),O=!0},{once:!0});let n=null,C=null,P=null,z=null,V=null;function N(){if(!n)try{n=new(window.AudioContext||window.webkitAudioContext);const e=n.createGain();e.gain.value=1,e.connect(n.destination);const M=n.createMediaElementSource(T);P=n.createGain(),P.gain.value=1,M.connect(P).connect(e),S.attachAnalyser(n,P),C=xa(n,e);const Z=n.sampleRate*2,re=n.createBuffer(1,Z,n.sampleRate),ve=re.getChannelData(0);for(let ce=0;ce<Z;ce++)ve[ce]=Math.random()*2-1;const le=n.createBufferSource();le.buffer=re,le.loop=!0;const se=n.createBiquadFilter();se.type="bandpass",se.frequency.value=320,se.Q.value=.7,z=n.createGain(),z.gain.value=0,le.connect(se).connect(z).connect(e),le.start(),V=ce=>{const ue=n.currentTime;for(const[de,Te,xe]of[[0,58,.5],[.19,46,.34]]){const ge=n.createOscillator();ge.type="sine",ge.frequency.setValueAtTime(Te*1.6,ue+de),ge.frequency.exponentialRampToValueAtTime(Te,ue+de+.06);const ye=n.createGain();ye.gain.setValueAtTime(0,ue+de),ye.gain.linearRampToValueAtTime(xe*ce*.4,ue+de+.012),ye.gain.exponentialRampToValueAtTime(1e-4,ue+de+.3),ge.connect(ye).connect(e),ge.start(ue+de),ge.stop(ue+de+.4)}}}catch(e){console.warn("[journey] web audio unavailable, voice only",e)}}let ee=null,H=null,J=!1,y=null,o=null,w=!1,p=null,G=null,f=!1;async function U(){try{const M=await(await fetch(q.url)).arrayBuffer();n&&(ee=await n.decodeAudioData(M))}catch(e){console.warn("[journey] segment-2 voice failed to load",e)}try{const M=await(await fetch(F.voiceUrl)).arrayBuffer();n&&(y=await n.decodeAudioData(M))}catch(e){console.warn("[journey] segment-3 voice failed to load",e)}try{const M=await(await fetch(oe.voiceUrl)).arrayBuffer();n&&(p=await n.decodeAudioData(M))}catch(e){console.warn("[journey] segment-4 voice failed to load",e)}}let E=!1,I=0;T.addEventListener("loadedmetadata",()=>{isFinite(T.duration)&&T.duration});let A=null;function B(){if(A!==null)return A;if(!E)return-_e;if(G!==null&&!f&&n){const e=oe.voiceAt+(n.currentTime-G);return I=performance.now()/1e3-e,e}if(o!==null&&!w&&n){const e=F.voiceAt+(n.currentTime-o);return I=performance.now()/1e3-e,e}if(H!==null&&!J&&n){const e=q.start+(n.currentTime-H);return I=performance.now()/1e3-e,e}return!T.ended&&T.currentTime>.01?(I=performance.now()/1e3-T.currentTime,T.currentTime):performance.now()/1e3-I}let R="flat";async function r(){if(!navigator.xr)return null;try{if(await navigator.xr.isSessionSupported("immersive-ar"))return"immersive-ar"}catch{}try{if(await navigator.xr.isSessionSupported("immersive-vr"))return"immersive-vr"}catch{}return null}async function Y(){N(),n&&n.state==="suspended"&&await n.resume(),U();const e=await r();if(e){const M={requiredFeatures:["local-floor"],optionalFeatures:["hand-tracking"]};try{const Z=await navigator.xr.requestSession(e,M);await t.xr.setSession(Z),R=e,Z.addEventListener("end",()=>{R="flat"})}catch(Z){console.warn("[journey] could not start",e,Z)}}document.body.classList.add("started"),E=!0,I=performance.now()/1e3+_e,setTimeout(()=>{T.play().catch(M=>console.warn("[journey] autoplay blocked",M))},_e*1e3)}let te=null;function ft(e){if(te)return te;let M=h(d(e,[q.breathIn[0],q.breathIn[0]+1.6]))*(1-h(d(e,[q.breathIn[1],q.breathIn[1]+1]))),Z=h(d(e,[q.breathOut[0],q.breathOut[0]+.25]))*(1-h(d(e,[q.breathOut[0]+.55,q.breathOut[0]+1.2])));if(e>q.loopFrom&&e<F.collide[0]){const re=(e-q.loopFrom)%q.loopPeriod,ve=q.loopAmp,le=ve*h(d(re,[0,1.8]))*(1-h(d(re,[4.4,5.6]))),se=ve*h(d(re,[6,6.3]))*(1-h(d(re,[6.7,7.4])));M=Math.max(M,le),Z=Math.max(Z,se)}return[M,Z]}const He=new Q;let Ye=-_e,Ie=0,qe=0,$e=-1;t.setAnimationLoop(()=>{const e=B(),M=ke.clamp(e-Ye,.001,.1);Ye=e;const Z=t.getContext(),re=Math.max(.5,Z.drawingBufferHeight/900),ve=t.xr.isPresenting?t.xr.getCamera():i;if(E&&H===null&&ee&&n&&e>=q.start&&A===null){const L=n.createBufferSource();L.buffer=ee,L.connect(P),L.onended=()=>{J=!0},L.start(),H=n.currentTime}if(E&&o===null&&y&&n&&e>=F.voiceAt&&A===null){const L=n.createBufferSource();L.buffer=y,L.connect(P),L.onended=()=>{w=!0},L.start(),o=n.currentTime}if(E&&G===null&&p&&n&&e>=oe.voiceAt&&A===null){const L=n.createBufferSource();L.buffer=p,L.connect(P),L.onended=()=>{f=!0},L.start(),G=n.currentTime}const[le,se]=ft(e),ce=h(d(e,F.collide)),ue=h(d(e,[F.blast,F.blast+.22]))*(1-h(d(e,[F.blast+.55,F.blast+1.8]))),de=d(e,[F.blast,F.blast+3.2]),Te=h(d(e,[F.throw[0],F.throw[0]+.6]))*(1-h(d(e,[F.throw[1]-1.2,F.throw[1]]))),xe=h(d(e,F.form)),ge=h(d(e,F.world)),ye=h(d(e,[F.flare[0],F.flare[0]+.7]))*(1-h(d(e,[F.flare[1]-.6,F.flare[1]+1.2]))),ze=h(d(e,F.open)),me=h(d(e,F.suck)),Ke=h(d(e,F.black)),ne=h(d(e,[F.blast,F.blast+1.3])),vt=h(d(e,[F.throw[1]-.5,F.throw[1]+3.5])),fe=h(d(e,oe.reveal)),Je=h(d(e,oe.walk)),dt=h(d(e,oe.watch)),Re=h(d(e,oe.release)),be=h(d(e,oe.white));m.reveal=fe,m.watch=dt*(1-Re),m.release=Re,m.update(e,ve),c.position.z=-Je*120,_.collide=ce,_.ring=de,_.rush=Te;const Oe=1-fe;_.form=Math.max(ge*.34,xe)*Oe,_.flare=ye*Oe,_.open=ze*Oe,_.suck=me*Oe,_.black=Math.max(Ke*(1-fe),0),_.flash=Math.max(ue,be*.92),_.update(e,re,ve);const mt=st(0,.55,h(d(e,ie.motesIn)))+st(0,.45,h(d(e,ie.motesFull)));b.fade=mt*(1-h(d(e,[ie.dim[0],ie.dim[0]+9]))),b.update(e,re),x.opacity=Math.pow(d(e,ie.dim),1.6),u.emerge=d(e,ie.stars),u.global=1-h(d(e,q.starsOut))+vt*(1-me)*.85,u.update(e,re);const Ze=h(d(e,q.palette)),Xe=h(d(e,q.full)),De=(1-.45*k.breathing)*(1-ne)*(1-ce*.8);s.hell=fe*(1-be),g.hell=fe*(1-be),s.emerge=h(d(e,ie.nebula))*(De+ne*.3*(1-me))+fe*.28*(1-be),s.palette=Ze*(1-ne*.85),s.full=Xe*(1-ne*.6),s.update(e),g.emerge=h(d(e,ie.nebula))*(.9*De+ne*.45*(1-me))+fe*.34*(1-be),g.palette=Ze*(1-ne*.85),g.full=Xe*(1-ne*.6),g.update(e*.55),v.emerge=h(d(e,q.curtains))*De,v.update(e);const ht=.05+le*.85-se*1;qe+=(ht-qe)*(1-Math.exp(-M*2.2)),Ie+=qe*M,s.radial=Ie,g.radial=Ie*.35,k.core=h(d(e,q.core))*(1-ne),k.field=h(d(e,q.field))*(1-ne),k.gateIn=le*(1-ne),k.gateOut=se*(1-ne),k.update(e,M,ve,re);const et=h(d(e,[ie.dim[0]+8,ie.dim[1]+8]))*(1-me);if(He.set(Math.sin(e*.031)*.55,Math.sin(e*.019+1.7)*.42,Math.cos(e*.024)*.55).multiplyScalar(et),e<oe.reveal[0]?c.position.copy(He):c.position.set(0,0,-Je*120),c.rotation.z=Math.sin(e*.013)*.03*et,S.fade=h(d(e,[-_e+.8,-.2]))*(1-h(d(e,wa))),S.update(e,M,ve),C)if(e<q.start-4){const L=h(d(e,ie.drone)),Se=1-.45*h(d(e,[90,118]));C.level=L*Se}else e<F.collide[0]?C.level=.1+.1*le-.04*se:e<oe.reveal[0]?C.level=.1+ce*.16+xe*.12+me*.4-Ke*.34:C.level=(.16+fe*.1)*(1-Re);if(z&&n&&A===null){const L=fe*.3*(1-Re),Se=be*.24,Ve=ce*.18+ue*1+Te*.55+me*.85+L+Se;z.gain.setTargetAtTime(Ve*.16,n.currentTime,.12)}if(V&&A===null&&e>F.drumFrom&&e<oe.mantra){let L;if(e<oe.reveal[0])L=1.45-xe*.35-ze*.25-me*.25;else{const Ve=Math.floor(e/1.6);L=1.35+Math.sin(Ve*12.9898)*43758.5453%1*.55}const Se=Math.floor(e/L);Se!==$e&&($e=Se,V(.55+xe*.3+ze*.45+me*.6+fe*.25))}t.render(a,i)});const Qe=await r();Qe==="immersive-ar"?Ae("Put your headset on and begin.","You will start in your own room."):Qe==="immersive-vr"?Ae("Passthrough unavailable — begin anyway.","Your room will be represented rather than seen."):Ae("Preview in browser","Open this page in the Quest browser for the full experience.");const Le=document.getElementById("begin");Le.disabled=!1,Le.addEventListener("click",async()=>{Le.disabled=!0,O||(Ae("Loading the voice…"),await new Promise(e=>{if(O)return e();const M=setInterval(()=>{O&&(clearInterval(M),e())},100);setTimeout(()=>{clearInterval(M),e()},6e3)})),await Y()}),window.JOURNEY={THREE:Ut,scene:a,camera:i,renderer:t,stars:u,nebula:s,voidShell:x,motes:b,narrator:S,prana:k,sec3:_,naraka:m,T:ie,SEG2:q,SEG3:F,SEG4:oe,seek:e=>{A=e},resume:()=>{A=null},look:(e,M=0)=>{i.rotation.set(M,e,0,"YXZ")},moveTo:(e,M,Z)=>{c.position.set(0,0,0),i.position.set(e,M,Z)},forceBreath:(e,M)=>{te=e===null?null:[e,M]},fakeLevel:e=>{S.uniforms.uLevel.value=e},dryStart:()=>{E=!0,I=performance.now()/1e3,document.body.classList.add("started")},state:()=>({t:B(),xrMode:R,voidOpacity:x.opacity,starEmerge:u.emerge,nebulaEmerge:s.emerge,orbFade:S.fade})},window.__JOURNEY_READY=!0}ya().catch(l=>{console.error(l),window.__JOURNEY_ERROR=String(l&&l.stack||l),Ae("Something went wrong.",String(l))});
